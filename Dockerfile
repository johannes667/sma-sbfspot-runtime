...
RUN apt-get update && apt-get install -y build-essential git make gcc g++ libbluetooth-dev libboost-all-dev ca-certificates && rm -rf /var/lib/apt/lists/*
WORKDIR /opt
...
ENV DEBIAN_FRONTEND=noninteractive PYTHONUNBUFFERED=1 CONFIG_DIR=/config DATA_DIR=/data WEB_PORT=8088
RUN apt-get update && apt-get install -y libbluetooth3 libboost-date-time1.74.0 libboost-system1.74.0 bluez bluez-tools mosquitto-clients ca-certificates bash coreutils grep sed gawk procps python3 python3-flask python3-pip && rm -rf /var/lib/apt/lists/*
