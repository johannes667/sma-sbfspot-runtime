...
ENV DEBIAN_FRONTEND=noninteractive PYTHONUNBUFFERED=1 CONFIG_DIR=/config DATA_DIR=/data
RUN apt-get update && apt-get install -y libbluetooth3 libboost-date-time1.74.0 libboost-system1.74.0 bluez bluez-tools mosquitto-clients ca-certificates bash coreutils grep sed gawk procps python3 python3-flask && rm -rf /var/lib/apt/lists/*
